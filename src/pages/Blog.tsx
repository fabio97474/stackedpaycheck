    ...
    <div>
      ...
      <div style={{ borderBottom: '1px solid #e2e8f0', backgroundColor: '#f9f8f5', padding: '0.75rem 0' }}>
        <div style={{ maxWidth: '1024px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', gap: '0.75rem', overflowX: 'auto' }}>
          {categories.map(c => (
            <span key={c} style={{ fontFamily: 'system-ui, sans-serif', fontSize: '0.75rem', fontWeight: 600, padding: '0.375rem 0.75rem', borderRadius: '9999px', backgroundColor: '#f0e8d4', color: '#a8853a', whiteSpace: 'nowrap', cursor: 'pointer' }}>{c}</span>
          ))}
        </div>
      </div>
          ...
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
              {posts.map(post => (
                <Link key={post.slug} to={`/posts/${post.slug}`} style={{ textDecoration: 'none' }}>
                  <article style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '0.5rem', padding: '1.5rem', height: '100%' }}>
